...
- **语音聊天闭环（流式）**：听写（Web Speech API）→ 大模型（DeepSeek，流式输出）→ 分句朗读 自动循环；模型一边生成、一边把成型的句子立刻读出来，孩子更快听到回应。
...
- **更快的响应**：DeepSeek 流式输出，句子成型即朗读，无需等整段生成完，首句延迟明显降低。
- **说话时也能随时打断（barge-in）**：AI 说话的同时也在听，孩子一开口就立刻停下、转去听孩子说（像真聊天一样插话）；也可点击脸手动打断。已做"开播前静默窗 + 回声过滤"降低误触发，设置里可关闭此开关。
...
│   └── app.js          # 主流程：流式状态机、分句朗读、随时打断、UI 联动
└── README.md
```
